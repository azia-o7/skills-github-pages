...
This is my final variation and my main showcase piece of all the experiments I have done. I went back to rectangle shapes and started to experiment trying to create a moving structure out of the smaller pieces. After much trial and error, I made my final structure. I kept the colour scheme black and white since I was inspired by the music video for Lagtrain and even the song somewhat inspired me as well (Source: https://youtu.be/UnIhRpIT7nc?feature=shared). Another music video that somewhat inspired the main structure of the piece was Patchwork Eden as they have 3D block like buildings and other shapes to create strange looking structures. Albeit, the music video has a more colourful tone to it (Source: https://youtu.be/ZlyQu4LGyxo?feature=shared). The piece also looks like TVs that have been stacked on top of each other which also links to the Patchwork Eden music video as there was a distorted TV character within that video as well. I liked how this one turned out and I had a lot of fun trying to experiment with creating the final structure to what I wanted it to look like based on my original idea. I could've tried to recreate the colour scheme of the Patchwork Eden music video but I did have problems with trying the colours out for the designs. I tried to get a rainbow design for the second variation but it changed them all to the same colour rather than different ones. I could've looked into it more and found a way to make them a pastel rainbow colour instead of black and white. I do like the Lagtrain theme I eventually went with for the end theme though.

The final alterations for this code was changing the outer **for** loop from **numAcross** to a flat number which is **360**. I changed the **translate** values to **multiply both by 2** and change the **multiplication** of the first equation to a **division** instead. I changed the rotate to a flat value of **30** instead of a **frameCount** value. I changed the multiplication of the **random(step)** values of the **rect** shape. I had to change the **x position** of them as well to **225** otherwise it would be off the screen to the left due to the step positions. Most of these values were changed a lot due to trial and error to cry and create the structure I wanted it to look like. These all helped create the main structure and animation that you see in the final piece.
...
function drawTile(across,down,step){
  push()
  translate((across*2)/step,(down*2)*step)
  angleMode(DEGREES)
  rotate(30)
  rect(225,0,random(step*10),random(step*5))
  pop()
}
```
